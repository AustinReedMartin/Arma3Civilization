...
if (_victim isEqualTo _killer) then
{
	...
	'ARMA_LOG' callExtension format['Exile_Esseker_A3_KILLED:%1',_log];
	["systemChatRequest", [format["%1 commited suicide! You lose 150 respect!", (name _victim)]]] call ExileServer_object_player_event_killFeed;
	_newScore = _victim getVariable ["ExileScore", 0];
	_newScore =  _newScore - 50;
